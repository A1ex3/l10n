...
class {{ .BaseClassName }} {
     ...
     * Example: <b>{{ .Example }}</b>
     */
    {{ .Name }}({{ range $i, $param := .Parameters }}{{ if $i }}, {{ end }}{{ $param.Name }}{{ end }}) {
...
class {{ .Name }} extends {{ $.BaseClassName }} {
    ...
    {{ $methodName }}({{ $methodParams := index $.MethodsByName $methodName }}{{ range $i, $param := $methodParams.Parameters }}{{ if $i }}, {{ end }}{{ $param.Name }}{{ end }}) {
...
class {{ .ClassName }} {
    static DEFAULT_LANGUAGE_CODE = "{{ .DefaultLangCode }}";
    static currentLanguageCode = "{{ .CurrentLangCode }}";
    static languages = {
    ...
    static setCurrentLanguageCode(languageCode) {
